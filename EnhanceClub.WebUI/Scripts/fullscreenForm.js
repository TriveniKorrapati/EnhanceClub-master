...
(function (window) {
  ...
  FForm.prototype.options = {
    ...
    allowNavigationOnEmpty: false,
    onFieldChange: function() { return; },
    // reached the review and submit step
    onReview() { return false; },
  };
  ...
  FForm.prototype._initEvents = function () {
    ...
    // show next field
    this.ctrlContinue.addEventListener('click', () => {
      self._nextField();
    });
    ...
    this.fields.forEach((fld) => {
      if (fld.hasAttribute('data-input-trigger')) {
        const input = fld.querySelector('input[type="radio"]') || /* fld.querySelector( '.cs-select' ) || */ fld.querySelector('select'); // assuming only radio and select elements (TODO: exclude multiple selects)
        if (!input) return;
  ...
  FForm.prototype._nextField = function (backto) {
    ...
    // current field
    const currentFld = this.fields[this.current];
  ...
  FForm.prototype._validade = function () {
    const fld = this.fields[this.current];
    const input = fld.querySelector('input[required]') || fld.querySelector('textarea[required]') || fld.querySelector('select[required]');
    let error;
    ...
    if (error != undefined) {
      if( this.options.allowNavigationOnEmpty && error == 'NOVAL' ){
        return true;
      }
